...
### PolicyPal (in progress): https://github.com/Badri-Rajendran/PolicyPal
A RAG-powered chatbot that answers insurance questions (health, auto, life, home and more) from a curated knowledge base, with cited sources. It refuses questions it can't ground instead of guessing.
- Hybrid retrieval: BM25 plus pgvector semantic search, reranked by a cross-encoder; every answer lists its source passages and relevance scores.
...
- Threaded conversations, JWT-authenticated API with per-user thread isolation, per-user daily token budget, 85% backend test-coverage floor.
- Stack: React 19 (Vite), Flask, PostgreSQL + pgvector, SQLAlchemy + Alembic, rank-bm25, sentence-transformers, Hugging Face transformers.
- In progress: plan comparison for ACA individual and family health plans from the CMS Marketplace API.

### CodeSage: Agentic Code Review Assistant (Mar 2026, completed): https://github.com/Badri-Rajendran/CodeSage
My flagship project: a Claude-powered multi-agent system that reviews pull requests the way a senior team would, reasoning, reflecting and citing codebase context before it comments.
